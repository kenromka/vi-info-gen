...
#============================================================================
        ...
        while not finish_flg:
            ...
            if less_cnt * more_cnt == 1 and oper['|'] != self._oper:
                ...
                finish_flg = False
                continue
            if (len(domain3) < 15 or
                 (
                    (self._minmax == minmax['max'] and max(domain3) >= (max(domain) - 15))
                     or
                    (self._minmax == minmax['min'] and min(domain3) <= (min(domain) + 15))
            )):
                finish_flg = False
                continue
...
SubtypeB = type('SubtypeB', (Type1,), B_dict)
