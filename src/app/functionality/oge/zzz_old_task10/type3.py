...
class Type3(Task10Old):
    """Подсчет суммы"""
    def __init__(self):
        super().__init__()

        self.question = choices(["В таб­ли­це Tur хра­нят­ся дан­ные из­ме­ре­ний сред­не­су­точ­ной "\
        "тем­пе­ра­ту­ры за {length} дней в гра­ду­сах (Tur[1] — дан­ные за пер­вый день, "\
        "Tur[2] — за вто­рой и т. д.). Определите, какое число будет на­пе­ча­та­но в"\
        " ре­зуль­та­те ра­бо­ты сле­ду­ю­щей программы. Текст про­грам­мы приведён на пяти язы­ках программирования.{table}",
        "В таблице Tur представлены данные о количестве голосов, поданных за {length} исполнителей народных"\
        " песен (Tur[1] – количество голосов, поданных за первого исполнителя; Tur[2] – за второго и т. д.)."\
    ...
    def category(self):
        return "Анализ алгоритмов - Подсчет суммы"
...
class SubtypeA(Type3):
    ...
    def __init__(self):
        ...
        for num, d in enumerate(dat):
            dat_init_pascal += f"    Tur[{num+1}] := {d};" if num % 2 == 0 else f" Tur[{num+1}] := {d};\n"
            dat_init_basic += f"Tur({num+1}) = {d}:" if num % 2 == 0 else f" Tur({num+1}) = {d}\n"
            dat_init_algo += f"Tur[{num+1}] := {d}\n"
        ...
        f"Tur: array[1..{self._length}] of integer;\n"\
        "begin\n"
        prog_pascal += dat_init_pascal
        prog_pascal += f"    m := {m};\n"\
        f"    for k := {start+1} to {end} do\n"\
        f"        if Tur[k] {sign} {elem_cnt} then\n"\
        "        begin\n"\
        "            m := m + Tur[k];\n"\
        "        end;\n"\
        "    writeln(m);\n"\
        ...
        prog_python = f"Tur = {dat_init_py}\n"\
        f"m = {m}\n"\
        f"for k in range({start}, {end}):\n"\
        f"    if Tur[k] {sign} {elem_cnt}:\n"\
        f"        m += Tur[k]\n"\
        f"print(m)\n"
        prog_cpp = "#include &lt;iostream&gt;\n"\
        "using namespace std;\n"\
        "int main() {\n"\
        f"    int Tur[{self._length}] = {dat_init_cpp}\n"\
        f"    int m = {m};\n"\
        f"    for (int k = {start}; k < {end}; k++)\n"\
        f"        if (Tur[k] {sign} {elem_cnt}) m += Tur[k];\n"\
        "    cout << m;\n"\
        "    return 0;\n"\
        "}\n"

        prog_basic = f"DIM Tur({self._length}) AS INTEGER\n"\
        ...
        prog_algo = "алг\n"\
        "нач\n"\
        f"целтаб Tur[1:{self._length}]\n"\
        "цел k, m\n"
        prog_algo += dat_init_algo
        prog_algo += f"m := {m}\n"\
        f"нц для k от {start+1} до {end}\n"\
        f"    если Tur[k] {sign} {elem_cnt} то\n"\
        f"        m := m + Tur[k]\n"\
        "    все\n"\
        "кц\n"\
        "вывод m\n"\
